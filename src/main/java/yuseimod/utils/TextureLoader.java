...
import static yuseimod.YuseiMod.cardPath;
import static yuseimod.YuseiMod.imagePath;
import static yuseimod.YuseiMod.logger;
import static yuseimod.YuseiMod.powerPath;
...
public class TextureLoader {
    ...
    {
        String textureString = cardPath(cardType.name().toLowerCase(Locale.ROOT) + "/" + cardName);
        logger.info("Attempting to load card texture: " + textureString);
        FileHandle h = Gdx.files.internal(textureString);
        if (!h.exists())
        {
            switch (cardType) {
                case ATTACK:
                    textureString = cardPath("attack/default");
                    break;
                case POWER:
                    textureString = cardPath("power/default");
                    break;
                default:
                    textureString = cardPath("skill/default");
                    break;
            }
        }
