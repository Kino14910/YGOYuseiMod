...
import static yuseimod.utils.GeneralUtils.removePrefix;
import static yuseimod.utils.TextureLoader.getCardTextureString;
...
import yuseimod.utils.CardStats;
import yuseimod.utils.TriFunction;


public abstract class YuseiCard extends AbstractSignatureCard  {
    final private static Map<String, DynamicVariable> customVars = new HashMap<>();

    protected static String makeID(String name) { return YuseiMod.makeID(name); }
    ...
    public YuseiCard(String ID, CardStats info) {
        this(ID, info, getCardTextureString(removePrefix(ID), info.cardType));
    }
    public YuseiCard(String ID, CardStats info, String cardImage) {
        this(ID, info.baseCost, info.cardType, info.cardTarget, info.cardRarity, info.cardColor, cardImage);
    }
    public YuseiCard(String ID, int cost, CardType cardType, CardTarget target, CardRarity rarity) {
        this(ID, cost, cardType, target, rarity, YGOCardColor.Monster);
    }
    public YuseiCard(String ID, int cost, CardType cardType, CardTarget target, CardRarity rarity, CardColor color) {
        this(ID, cost, cardType, target, rarity, color, getCardTextureString(removePrefix(ID), cardType));
    }
    public YuseiCard(String ID, int cost, CardType cardType, CardTarget target, CardRarity rarity, CardColor color, String cardImage)
    {
        super(ID, getName(ID), cardImage, cost, getInitialDescription(ID), cardType, color, rarity, target);
        System.out.println("Creating card: " + cardImage);
    ...
    protected final void setCustomVar(String key, VariableType type, int base, TriFunction<YuseiCard, AbstractMonster, Integer, Integer> preCalc) {
        setCustomVar(key, type, base, 0, preCalc);
    }
    protected final void setCustomVar(String key, VariableType type, int base, int upgrade, TriFunction<YuseiCard, AbstractMonster, Integer, Integer> preCalc) {
        setCustomVar(key, type, base, upgrade, preCalc, LocalVarInfo::noCalc);
    }
    protected final void setCustomVar(String key, VariableType type, int base, TriFunction<YuseiCard, AbstractMonster, Integer, Integer> preCalc, TriFunction<YuseiCard, AbstractMonster, Integer, Integer> postCalc) {
        setCustomVar(key, type, base, 0, preCalc, postCalc);
    }
    protected final void setCustomVar(String key, VariableType type, int base, int upgrade, TriFunction<YuseiCard, AbstractMonster, Integer, Integer> preCalc, TriFunction<YuseiCard, AbstractMonster, Integer, Integer> postCalc) {
        setCustomVarValue(key, base, upgrade);
    ...
    protected void setVarCalculation(String key, TriFunction<YuseiCard, AbstractMonster, Integer, Integer> calculation) {
    ...
    public AbstractCard makeStatEquivalentCopy() {
        ...
        if (candidate instanceof YuseiCard) {
            YuseiCard card = (YuseiCard) candidate;
    ...
    public void upgrade()
    {
        if (!upgraded)
        {
            ...
            if (this.upgradesDescription)
            {
                ...
                {
                    YuseiMod.logger.error("Card " + cardID + " upgrades description and has null upgrade description.");
                }
                else
                {
    ...
    private static class QuickDynamicVariable extends DynamicVariable {
        ...
        private YuseiCard current = null;
        ...
        public void setIsModified(AbstractCard c, boolean v) {
            if (c instanceof YuseiCard) {
                LocalVarInfo var = ((YuseiCard) c).getCustomVar(localKey);
                if (var != null)
                    var.forceModified = v;
            }
        }
        ...
        public boolean isModified(AbstractCard c) {
            return c instanceof YuseiCard && (current = (YuseiCard) c).isCustomVarModified(localKey);
        }

        @Override
        public int value(AbstractCard c) {
            return c instanceof YuseiCard ? ((YuseiCard) c).customVar(localKey) : 0;
        }

        @Override
        public int baseValue(AbstractCard c) {
            return c instanceof YuseiCard ? ((YuseiCard) c).customVarBase(localKey) : 0;
        }

        @Override
        public boolean upgraded(AbstractCard c) {
            return c instanceof YuseiCard && ((YuseiCard) c).customVarUpgraded(localKey);
        }
    ...
    protected static class LocalVarInfo {
        ...
        TriFunction<YuseiCard, AbstractMonster, Integer, Integer> calculation = LocalVarInfo::noCalc;
        ...
        private static int noCalc(YuseiCard c, AbstractMonster m, int base) {
            return base;
        }
